...
async function runOneOffCheckInBackground(id, domain, keyword, language, platform, customPrompt = null) {
    const { mentioned, context, citations, ownPosition, ownPositionTotal } = await checkPlatformMention(platform, keyword, domain, language, 'empfehlung', customPrompt)
    ...
    await GeoOneoffCheck.findByIdAndUpdate(id, { status: 'sentiment', 'result.mentioned': true, 'result.context': context, 'result.citations': citations || [], 'result.ownPosition': ownPosition ?? null, 'result.ownPositionTotal': ownPositionTotal ?? null })
...
export async function getCheckStatus(req, res) {
    ...
    res.json({
        ...
        prompt: doc.prompt,
        ...(doc.status === 'done' ? { mentioned: doc.result.mentioned, context: doc.result.context, sentiment: doc.result.sentiment, citations: doc.result.citations || [], ownPosition: doc.result.ownPosition ?? null, ownPositionTotal: doc.result.ownPositionTotal ?? null } : {}),
    })
}
