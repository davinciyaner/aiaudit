...
export default function RegisterPageEn() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setTimeout(() => router.push(geoCheckContext ? '/en/geo/pricing' : '/dashboard'), 1000)
        } catch (err) {
                            ...
                            <div className="flex items-center gap-2 mt-3 px-3 py-2 bg-[var(--accent-soft)] border border-[var(--accent-border)] rounded-xl">
                                <p className="text-xs text-[var(--accent)]">
                                    {geoCheckContext.mentioned
                                        ? <>You're cited by <b>{geoCheckContext.label}</b> — stay on top of it with weekly tracking for <b>{geoCheckContext.domain}</b>.</>
                                        : <>You're not cited by <b>{geoCheckContext.label}</b> yet — start tracking <b>{geoCheckContext.domain}</b> to change that.</>}
                                </p>
                            </div>
                        ) : hasPendingAudit && (
                            <div className="flex items-center gap-2 mt-3 px-3 py-2 bg-[var(--accent-soft)] border border-[var(--accent-border)] rounded-xl">
