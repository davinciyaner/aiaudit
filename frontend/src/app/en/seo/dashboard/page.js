...
function SiteCard({ site, onDelete }) {
    ...
    const handleDelete = async (e) => {
        e.preventDefault()
        if (!confirm(`Delete "${site.domain}" and all rankings?`)) return
        setDeleting(true)
        try {
...
export default function SeoDashboardPageEn() {
    ...
    useEffect(() => {
        ...
        if (!stored) { router.push('/en/login?redirect=/en/seo/dashboard'); return }
        fetchSites()
    }, [fetchSites, router])
