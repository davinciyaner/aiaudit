...
function LockedIssues({ count, type = 'error', onRegister }) {
    ...
    const fakeTexts = [
        'Another critical issue found on this page.',
        'Error affects your Google visibility.',
            ...
            <div className="max-w-5xl mx-auto px-4 sm:px-8 pt-28 pb-8 sm:pb-12">
                ...
                {result?.limitReached && (
                    ...
                    >
                        ...
                        <p className="text-xs text-slate-600">Next free check: in one month</p>
                    </motion.div>
                )}
                            ...
                            <>
                                        ...
                                        {[
                                            {
                                                ...
                                                detail: audit?.seo?.title?.length ? `${audit.seo.title.length} characters` : 'missing',
                                            },
                                            {
                                                label: 'Meta Description',
                                                ok: audit?.seo?.description?.length >= 120 && audit?.seo?.description?.length <= 160,
                                                detail: audit?.seo?.description?.length ? `${audit.seo.description.length} characters` : 'missing',
                                            },
                                            {
                                                label: 'H1 Tag',
                                                ok: audit?.seo?.headings?.h1?.length === 1,
                                                detail: `${audit?.seo?.headings?.h1?.length ?? 0}× found`,
                                            },
                                            {
                                                label: 'Alt Text',
                                                ok: audit?.seo?.images?.withoutAlt === 0,
                                                detail: audit?.seo?.images?.withoutAlt ? `${audit.seo.images.withoutAlt} missing` : 'all present',
                                            },
                                            {
                                                label: 'Canonical',
                                                ok: !!audit?.seo?.canonical,
                                                detail: audit?.seo?.canonical ? 'set' : 'missing',
                                            },
                                            {
                                                label: 'Structured Data',
                                                ok: !!audit?.seo?.structuredData,
                                                detail: audit?.seo?.structuredData ? 'present' : 'missing',
                                            },
                                        ].map(({ label, ok, detail }) => (
                                ...
                                {audit?.geo && (
                                    <Section title="GEO — AI Visibility" icon="🤖">
                                        ...
                                        <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 mb-5">
                                            {[
                                                { label: 'llms.txt', ok: audit.geo.checks?.hasLlmsTxt, detail: audit.geo.checks?.hasLlmsTxt ? 'present' : 'missing' },
                                                { label: 'llms-full.txt', ok: audit.geo.checks?.hasLlmsFullTxt, detail: audit.geo.checks?.hasLlmsFullTxt ? 'present' : 'missing' },
                                                { label: 'FAQ Schema', ok: audit.geo.checks?.hasFAQ, detail: audit.geo.checks?.hasFAQ ? 'present' : 'missing' },
                                                { label: 'Organization', ok: audit.geo.checks?.hasOrganization, detail: audit.geo.checks?.hasOrganization ? 'present' : 'missing' },
                                                { label: 'AI Crawlers', ok: audit.geo.checks?.robotsAllowsAI, detail: audit.geo.checks?.robotsAllowsAI ? 'allowed' : `${audit.geo.checks?.blockedCrawlers?.length ?? 0} blocked` },
                                                { label: 'sitemap.xml', ok: audit.geo.checks?.hasSitemap, detail: audit.geo.checks?.hasSitemap ? 'present' : 'missing' },
                                                { label: 'Product Definition', ok: audit.geo.checks?.hasDirectDefinition, detail: audit.geo.checks?.hasDirectDefinition ? 'found' : 'missing' },
                                                { label: 'Statistics', ok: audit.geo.checks?.hasStatistics, detail: audit.geo.checks?.hasStatistics ? 'present' : 'missing' },
                                                { label: 'lang Attribute', ok: audit.geo.checks?.hasLang, detail: audit.geo.checks?.hasLang ? 'set' : 'missing' },
                                                { label: 'HTTPS', ok: audit.geo.checks?.hasHTTPS, detail: audit.geo.checks?.hasHTTPS ? 'active' : 'missing' },
                                                { label: 'Canonical', ok: audit.geo.checks?.canonical, detail: audit.geo.checks?.canonical ? 'set' : 'missing' },
