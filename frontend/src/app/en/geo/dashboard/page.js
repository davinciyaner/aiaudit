...
function SiteCard({ site, onDelete }) {
    ...
    const handleDelete = async (e) => {
        e.preventDefault()
        if (!confirm(`Delete "${site.domain}" and all GEO data?`)) return
        setDeleting(true)
        try {
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            ...
            >
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div>
                        <label className="text-sm text-slate-300 font-medium block mb-1.5">Track AI platforms</label>
                        ...
                        <div className="flex items-center justify-between mt-1">
                            ...
                            {kwCount > 0 && (
                                <p className="text-xs text-slate-500">
                                    ~<span className="text-slate-300 font-medium">${estimatedCost}</span>/month API cost
                                </p>
                            )}
                        </div>
                    </div>
...
export default function GeoDashboardPageEn() {
    return (
        <Suspense fallback={null}>
            <GeoDashboardPageEnInner />
        </Suspense>
    )
}

function GeoDashboardPageEnInner() {
            ...
            <div className="max-w-7xl mx-auto px-5 sm:px-8 pt-28 pb-16">
                ...
                <div className="flex items-start justify-between gap-4 mb-8">
                    <div>
                        ...
                        <h1 className="text-2xl sm:text-3xl font-bold text-white">GEO Automation</h1>
                        <p className="text-slate-500 text-sm mt-1">Is your domain mentioned by AI systems?</p>
                    </div>
