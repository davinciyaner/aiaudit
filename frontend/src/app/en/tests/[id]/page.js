...
export default function TestResultPageEn() {
    const { id } = useParams()
                    ...
                    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
                        ...
                        {data.status === 'running' && (
                            <div className="flex items-center gap-3 bg-amber-500/8 border border-amber-500/20 rounded-2xl px-5 py-4">
                                ...
                                <p className="text-amber-400 text-sm">Test is running — this page updates automatically…</p>
                            </div>
                        )}
                        ...
                        {data.summary && (
                            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                                ...
                                <StatCard icon={Clock}          label="Duration" value={`${(data.summary.duration / 1000).toFixed(1)}s`} color="bg-violet-500/15 text-violet-400" />
                            </div>
                        )}
