...
const RANKINGS = [
    { keyword: 'free seo test',          pos: 3,  prev: 7,  vol: '8.1K', url: 'sitecheckai.dev',          ctr: '11%' },
    ...
    { keyword: 'seo analysis online',    pos: 18, prev: 9,  vol: '5.2K', url: 'sitecheckai.dev/blog/seo', ctr: '1%'  },
    { keyword: 'check meta description', pos: 2,  prev: 5,  vol: '1.9K', url: 'sitecheckai.dev',          ctr: '15%' },
    { keyword: 'lighthouse alternative', pos: 7,  prev: 7,  vol: '3.4K', url: 'sitecheckai.dev',          ctr: '4%'  },
]

const MOCK_FILTERS = ['All', 'Top 3', 'Top 10', '↑ Up', '↓ Down']

const IDEAS = [
    { keyword: 'free seo check tool',    vol: '12.4K', comp: 'HIGH',   cpc: '2.40' },
    { keyword: 'website speed test',     vol: '6.8K',  comp: 'MEDIUM', cpc: '1.10' },
    { keyword: 'check google ranking',   vol: '5.2K',  comp: 'MEDIUM', cpc: '0.90' },
    { keyword: 'meta tags generator',    vol: '3.7K',  comp: 'LOW',    cpc: '0.60' },
    { keyword: 'h1 tag checker',         vol: '2.1K',  comp: 'LOW',    cpc: '0.40' },
]
...
const BACKLINKS = [
    { label: 'Total backlinks',    value: '14,382', color: 'text-white' },
...
const BENEFITS = [
    {
        ...
        desc: 'Your rankings are automatically refreshed every Monday for all keywords — zero manual effort.',
    },
    {
        icon: Lightbulb, color: '#f59e0b',
        title: 'Keyword ideas & volume',
        desc: 'Search volume, competition strength and CPC for your keywords — plus 20 new ideas at the click of a button.',
    },
    {
        icon: Users, color: '#06b6d4',
        title: 'Competitor analysis',
        desc: 'See which domains rank for your keywords, how many shared keywords you have, and where they outrank you.',
    },
    {
        icon: Bell, color: '#10b981',
        title: 'Alerts & CSV export',
        desc: 'Email alerts on ranking drops — fully configurable. Export rankings as CSV whenever you want.',
    },
]
                ...
                <table className="w-full">
                    <thead>
                        <tr className="border-b border-white/[0.05]">
                            {['Keyword', 'Position', 'Change', 'CTR (est.)', 'Volume', 'URL'].map(h => (
    ...
    return (
        <div className="overflow-x-auto">
            <div className="px-5 py-2.5 mb-1">
                <span className="text-[10px] font-semibold uppercase tracking-wider text-emerald-500">New keyword ideas</span>
            </div>
            <table className="w-full">
                <thead>
                    <tr className="border-b border-white/[0.05]">
                        {['Keyword', 'Volume/month', 'Competition', 'CPC', ''].map(h => (
                            <th key={h} className="px-5 py-3 text-left text-[10px] font-semibold text-slate-600 uppercase tracking-wider whitespace-nowrap">{h}</th>
                        ))}
                    </tr>
                </thead>
                            ...
                            <td className="px-5 py-3.5">
                                {i === 1 ? (
                                    ...
                                    <span className="flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-semibold bg-white/5 text-slate-400 border border-white/10 cursor-default">
                                        <TrendingUp className="w-3 h-3" />Track
                                    </span>
                                )}
                            </td>
                        </tr>
    ...
    return (
        <div className="overflow-x-auto">
            <table className="w-full">
                <thead>
                    <tr className="border-b border-white/[0.05]">
                        {['#', 'Domain', 'Shared KW', 'Top 10', 'Top 3'].map(h => (
                            <th key={h} className="px-5 py-3 text-left text-[10px] font-semibold text-slate-600 uppercase tracking-wider whitespace-nowrap">{h}</th>
                        ))}
                    </tr>
                </thead>
    ...
    return (
        <div className="p-5">
            ...
            <div className="bg-white/[0.02] border border-white/[0.05] rounded-xl p-4">
                <div className="text-[10px] text-slate-600 uppercase tracking-wider mb-3">Backlink profile overview</div>
                ...
                <div className="text-[10px] text-slate-700 mt-2">New backlinks — last 12 months</div>
            </div>
        </div>
    )
}
            ...
            <div className="relative max-w-7xl mx-auto px-5 sm:px-8">
                ...
                <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="mb-10">
                    <h2 className="text-3xl sm:text-4xl lg:text-5xl font-bold tracking-tight leading-tight mb-4">
                        ...
                        <span className="text-transparent bg-clip-text bg-gradient-to-r from-emerald-400 to-teal-400">
                            automated fresh every week.
                        </span>
                    </h2>
                    <p className="text-slate-400 text-base sm:text-lg leading-relaxed max-w-2xl">
                        Instead of checking Google Search Console manually: rankings, keyword ideas, competitors and backlinks — all automatic, all in one place.
                    </p>
                </motion.div>
                ...
                >
                    ...
                    <div className="flex items-center justify-between px-5 py-4 border-b border-white/[0.05] bg-white/[0.01]">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <div className="text-[11px] text-slate-600">500 keywords · weekly</div>
                            </div>
                        </div>
                        <div className="flex items-center gap-3">
                            <div className="flex gap-4 text-center">
                                {[['3', 'Top 3', 'text-emerald-400'], ['12', 'Top 10', 'text-teal-400'], ['+8', 'Winners', 'text-cyan-400'], ['−2', 'Losers', 'text-red-400']].map(([v, l, c]) => (
                                    <div key={l}>
                    ...
                    <div className="px-5 py-3 border-t border-white/[0.05] flex items-center justify-between bg-white/[0.01]">
                        <span className="text-[11px] text-slate-700">250 of 500 keywords · my-website.com · AuditAI SEO Automation</span>
                        <span className="text-[11px] text-emerald-600 font-medium cursor-default">Show all 500 →</span>
                    </div>
                </motion.div>
                ...
                <div className="grid sm:grid-cols-3 gap-4 mb-16">
                    ...
                    <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.15 }}
                        ...
                        <div className="px-4 py-3 border-b border-white/[0.05] flex items-center gap-2">
                            ...
                            <span className="text-sm font-semibold text-white">Keyword Ideas</span>
                            <span className="ml-auto text-[10px] text-amber-500/60 font-semibold uppercase tracking-wide">20 new</span>
                        </div>
