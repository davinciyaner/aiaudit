...
const MENTIONS = [
    ...
    { keyword: 'llms.txt setup', claude: true, chatgpt: false, perplexity: true, google_aio: false },
...
const COMPETITORS = [
    { domain: 'seo-tool-compare.com', share: 18.4, platforms: 4 },
...
const CORRELATION = [
    { keyword: 'seo tool', seoPos: 4, geo: true, status: 'Both', color: 'text-emerald-400 bg-emerald-500/10 border-emerald-500/20' },
    { keyword: 'website audit', seoPos: 12, geo: false, status: 'SEO only', color: 'text-amber-400 bg-amber-500/10 border-amber-500/20' },
    { keyword: 'generative engine optimization', seoPos: null, geo: true,  status: 'AI only', color: 'text-blue-400 bg-blue-500/10 border-blue-500/20' },
    { keyword: 'ai seo audit', seoPos: null, geo: false, status: 'Neither', color: 'text-slate-500 bg-white/5 border-white/10' },
]

const SUGGESTIONS = [
    { keyword: 'seo tool', vol: 516 },
    { keyword: 'generative engine optimization', vol: 240 },
    { keyword: 'llms.txt setup', vol: 88 },
...
const TABS = [
    ...
    { id: 'correlation', label: 'SEO Ranking + AI Mentions',  icon: GitCompare },
    { id: 'suggestions', label: 'Suggest Keywords',           icon: Lightbulb },
]
...
const BENEFITS = [
    {
        ...
        desc: 'See exactly which other domains AI models cite alongside you — and on which platform, for which keyword.',
    },
    {
        icon: RefreshCw, color: '#a78bfa',
        title: 'Automatic, every week',
        desc: 'Claude, ChatGPT, Perplexity and Google AI Overview are checked automatically every week — with real recommendation prompts, not synthetic tests.',
    },
    {
        icon: GitCompare, color: '#34d399',
        title: 'SEO + AI Visibility',
        desc: 'Ranking on Google but never mentioned by AI — or the other way around? Only possible because both data sources live in the same system.',
    },
    {
        icon: Lightbulb, color: '#fbbf24',
        title: 'Real AI search volume',
        desc: 'Keyword suggestions based on real AI query volume, not just classic Google search volume.',
    },
]
                            ...
                            <tr key={i} className="border-b border-white/[0.04] last:border-0 hover:bg-white/[0.02] transition-colors">
                                ...
                                {PLATFORMS.map(p => (
                                    <td key={p} className="px-4 py-3.5">
                                        {m[p]
                                            ? <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[10px] font-semibold bg-emerald-500/10 text-emerald-400 border border-emerald-500/20"><Check className="w-2.5 h-2.5" />Yes</span>
                                            : <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[10px] font-semibold bg-white/5 text-slate-500 border border-white/10"><X className="w-2.5 h-2.5" />No</span>}
                                    </td>
                                ))}
                            </tr>
                        ))}
                ...
                >
                    <div className="flex items-center gap-3 px-5 py-3.5 border-b border-white/[0.06] bg-white/[0.02]">
                        ...
                        <div className="flex-1 flex items-center gap-2">
                            <div className="flex-1 bg-white/[0.04] border border-white/[0.06] rounded-md px-3 py-1.5 text-xs text-slate-600 flex items-center gap-2">
                                ...
                                sitecheckai.dev/en/geo/dashboard
                            </div>
                        </div>
                    ...
                    <div className="flex items-center justify-between px-5 py-4 border-b border-white/[0.05] bg-white/[0.01]">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <div className="text-sm font-semibold text-white">my-website.com</div>
                                <div className="text-[11px] text-slate-600">30 keywords · 4 platforms · weekly</div>
                            </div>
                        </div>
                    ...
                    <div className="px-5 py-3 border-t border-white/[0.05] flex items-center justify-between bg-white/[0.01]">
                        <span className="text-[11px] text-slate-700">5 of 30 keywords · my-website.com · AuditAI AI Visibility</span>
                        <span className="text-[11px] text-violet-600 font-medium cursor-default">View all 30 →</span>
                    </div>
                </motion.div>
                ...
                <div className="grid sm:grid-cols-3 gap-4 mb-16">
                    ...
                    <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.15 }}
                        ...
                        <div className="px-4 py-3 border-b border-white/[0.05] flex items-center gap-2">
                            ...
                            <span className="ml-auto text-[10px] text-cyan-500/60 font-semibold uppercase tracking-wide">Competitors</span>
                        </div>
                            ...
                            {COMPETITORS.map((c, i) => (
                                <div key={i} className="px-4 py-2.5 flex items-center justify-between gap-3">
                                    ...
                                    <div className="flex items-center gap-2 shrink-0">
                                        <span className="text-[10px] text-slate-500">{c.platforms} platf.</span>
                    ...
                    <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.22 }}
                        ...
                        <div className="px-4 py-3 border-b border-white/[0.05] flex items-center gap-2">
                            ...
                            <h3 className="text-sm font-semibold text-white">SEO + AI Mention</h3>
                        </div>
                        <div className="divide-y divide-white/[0.04]">
                            {CORRELATION.map((c, i) => (
                                <div key={i} className="px-4 py-2.5 flex items-center justify-between gap-3">
                    ...
                    <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.29 }}
                        ...
                        <div className="px-4 py-3 border-b border-white/[0.05] flex items-center gap-2">
                            ...
                            <h3 className="text-sm font-semibold text-white">Suggest Keywords</h3>
                            <span className="ml-auto text-[10px] text-amber-500/60 font-semibold uppercase tracking-wide">AI volume</span>
                        </div>
                        <div className="divide-y divide-white/[0.04]">
                            {SUGGESTIONS.map((s, i) => (
                                <div key={i} className="px-4 py-2.5 flex items-center justify-between gap-3">
                                    <span className="text-xs text-slate-300 truncate">{s.keyword}</span>
                                    <span className="text-[10px] text-cyan-400 font-semibold shrink-0">{s.vol.toLocaleString('en-US')} AI searches/mo.</span>
                                </div>
                            ))}
                        </div>
                    </motion.div>
