...
import { Puzzle, Play, CheckCircle, Zap, RefreshCw, ShieldCheck, Download, FileText, Eye, AlertTriangle, Mail } from 'lucide-react'
    ...
    return (
        <div className="w-64 bg-[#0d1117] border border-white/10 rounded-2xl shadow-2xl overflow-hidden">
            <div className="px-4 py-3 border-b border-white/[0.06] flex items-center justify-between">
                ...
                <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full ${active ? 'bg-red-500/15 text-red-400' : 'bg-emerald-500/15 text-emerald-400'}`}>
            ...
            <div className="px-3 pb-3 flex gap-2">
                <div className={`flex-1 py-2 rounded-xl text-[10px] font-semibold text-center ${active ? 'bg-red-500/15 text-red-400 border border-red-500/20' : 'bg-violet-500/15 text-violet-400 border border-violet-500/20'}`}>
                    {active ? '⏹ Stoppen' : '⏺ Aufnahme starten'}
                </div>
                {!active && visibleSteps > 0 && (
                    <div className="py-2 px-2.5 rounded-xl text-[10px] font-semibold bg-cyan-500/15 text-cyan-400 border border-cyan-500/20">
                        ↓ CSV
                    </div>
                )}
            </div>
    ...
    return (
        <div className="w-64 bg-[#0d1117] border border-white/10 rounded-2xl shadow-2xl overflow-hidden">
            <div className="px-4 py-3 border-b border-white/[0.06] flex items-center justify-between">
                ...
                <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full bg-emerald-500/15 text-emerald-400">Fertig</span>
            </div>
            ...
            <div className="px-3 pb-3 space-y-1">
                {items.map((item, i) => (
                    <div key={i} className={`flex items-center gap-2 px-2.5 py-1.5 rounded-lg border ${
                        ...
                        <span className={`text-[10px] ${item.result === 'pass' ? 'text-emerald-400' : item.result === 'warn' ? 'text-amber-400' : 'text-red-400'}`}>
...
function WaitlistForm() {
    ...
    return (
        <div className="max-w-lg mx-auto text-center">
            <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full border border-amber-500/20 bg-amber-500/5 text-amber-400 text-xs font-medium mb-5">
                Bald im Chrome Web Store
            </div>
            <h3 className="text-xl font-bold text-white mb-2">Früh-Zugang sichern</h3>
            <p className="text-slate-500 text-sm mb-6 leading-relaxed">
                Die Extension ist noch nicht öffentlich verfügbar. Trag deine E-Mail ein — wir benachrichtigen dich sobald sie live ist.
            </p>
...
export default function ExtensionSection() {
    ...
    useEffect(() => {
        const cycle = () => {
            ...
            setTimeout(() => { setRecording(false); setShowResult(true) }, 5000)
            ...
            <div className="max-w-7xl mx-auto px-5 sm:px-8 relative">
                ...
                <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
                    ...
                    <motion.div animate={{ opacity: showResult ? 1 : 0.2, x: showResult ? 0 : -6 }}
                        transition={{ duration: 0.4 }} className="text-slate-600 text-2xl hidden sm:block">→</motion.div>
                ...
                <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-24">
                    {STEPS.map((step, i) => (
                        <motion.div key={step.num}
                            ...
                            <span className="absolute top-4 right-5 text-4xl font-black text-white/[0.04] select-none">{step.num}</span>
                    ...
                    <div className="grid lg:grid-cols-2 gap-10 items-center">
                        <div>
                            <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full border border-cyan-500/20 bg-cyan-500/5 text-cyan-400 text-xs font-medium mb-5">
                                <FileText className="w-3 h-3" /> CSV Export
                            </div>
                            <h3 className="text-2xl sm:text-3xl font-bold text-white mb-4 leading-tight">
                                Jeder Schritt als<br />
                                <span className="text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-violet-400">lesbare CSV-Datei</span>
                            </h3>
                            ...
                            <ul className="space-y-3">
                                {[
                                    ...
                                    <li key={col} className="flex items-start gap-3">
                                        <code className="text-[10px] font-mono bg-white/[0.06] border border-white/10 text-violet-300 px-2 py-1 rounded shrink-0 mt-0.5">{col}</code>
                        ...
                        <div className="bg-[#0d1117] border border-white/10 rounded-2xl overflow-hidden shadow-2xl">
                            ...
                            <div className="px-4 py-2 border-b border-white/[0.04] grid grid-cols-4 gap-2">
                                {['step', 'action', 'selector', 'optional'].map(h => (
                                    <span key={h} className="text-[9px] font-semibold text-slate-600 uppercase tracking-wider font-mono">{h}</span>
                                ))}
                            </div>
                            {/* Data rows */}
                            <div className="divide-y divide-white/[0.03]">
                                {CSV_ROWS.map((row) => (
                                    <div key={row.step} className={`px-4 py-2 grid grid-cols-4 gap-2 items-center ${row.optional === 'true' ? 'bg-amber-500/[0.03]' : ''}`}>
                                        ...
                                        <span className={`text-[10px] font-mono ${row.optional === 'true' ? 'text-amber-400' : 'text-slate-700'}`}>
                                            {row.optional}
                                        </span>
                                    </div>
                                ))}
                            ...
                            <div className="px-4 py-3 border-t border-white/[0.04]">
                                <p className="text-[10px] text-slate-600">6 Schritte aufgezeichnet · editierbar in Excel, VS Code oder direkt hier</p>
                            </div>
                        </div>
                    </div>
                </motion.div>
                ...
                <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="mb-20">
                    ...
                    <p className="text-slate-500 text-center text-sm mb-10 max-w-xl mx-auto">
                        Manuelle Tests sind teuer, vergesslich und skalieren nicht. Automatisierte Tests ohne Code waren bisher nur für Entwickler.
                    </p>
