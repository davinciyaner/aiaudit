...
export default function RegistrationGate({ stats = [], auditUrl = '' }) {
    ...
    return (
        <motion.div
            ...
            className="relative overflow-hidden rounded-2xl border border-violet-500/25 bg-[var(--bg-base)] p-5 sm:p-8 md:p-12 text-center shadow-2xl shadow-violet-500/10"
        >
            ...
            <div className="relative z-10 max-w-lg mx-auto">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-8">
                    {LOCKED_FEATURES.map(({ icon: Icon, label }) => (
                        <div key={label} className="flex flex-col items-center gap-2 p-3 bg-[var(--text-white)]/[0.03] border border-[var(--text-white)]/[0.06] rounded-xl">
                            <Icon className="w-5 h-5 text-[var(--text-muted)]" />
                            <span className="text-[11px] text-[var(--text-faint)] leading-tight">{label}</span>
                        </div>
                    ))}
                </div>
                ...
                <div className="flex flex-col sm:flex-row gap-3 justify-center">
                    <button
                        ...
                        className="flex items-center justify-center gap-2 px-6 py-3 bg-gradient-to-r from-violet-600 to-cyan-600 hover:from-violet-500 hover:to-cyan-500 text-[var(--text-white)] font-semibold rounded-xl transition-all shadow-lg shadow-violet-500/20"
                    >
                    ...
                    <Link
                        href="/pricing"
                        className="flex items-center justify-center gap-2 px-6 py-3 text-[var(--text-body)] hover:text-[var(--text-white)] border border-[var(--text-white)]/10 hover:border-violet-500/40 text-sm font-semibold rounded-xl transition-all"
                    >
                        Pro für 29€/Monat
                ...
                <Link href="/login" className="block mt-5 text-xs text-[var(--text-faint)] hover:text-[var(--text-muted)] transition-colors">
